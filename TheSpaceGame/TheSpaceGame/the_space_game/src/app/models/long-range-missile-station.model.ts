...
export class LongRangeMissileStation extends PlaceableBuilding {
    ...
    constructor(position: Vector2D, level: number = 1) {
        super(LongRangeMissileStation.BUILD_COSTS, LongRangeMissileStation.DAMAGE_POINTS, 1, position,
            LongRangeMissileStation.DAMAGE_POINTS, LongRangeMissileStation.VECTOR_WIDTH, 
            LongRangeMissileStation.VECTOR_HEIGHT, LongRangeMissileStation.UPGRADE_COSTS);
    ...
    override draw(ctx: CanvasRenderingContext2D): void {
        ...
        if (lrmsImage.complete) {
            ctx.fillStyle = 'white';
            ctx.drawImage(lrmsImage, this.position.x, this.position.y, 50, 50);

            ctx.fillText("LRMS", this.position.x, this.position.y - 5);

...
export class Miner extends PlaceableBuilding {
    ...
    constructor(
        ...
        super(Miner.BUILD_COSTS, Miner.DAMAGE_POINTS, 1, position, Miner.DAMAGE_POINTS, Miner.VECTOR_WIDTH, Miner.VECTOR_HEIGHT, Miner.UPGRADE_COSTS);
    }
    ...
    override draw(ctx: CanvasRenderingContext2D): void {
        ...
        if (minerImage.complete) {
            ctx.fillStyle = 'white';
            ctx.drawImage(minerImage, this.position.x, this.position.y, 50, 50);

            ctx.fillText("Miner", this.position.x, this.position.y - 5);
